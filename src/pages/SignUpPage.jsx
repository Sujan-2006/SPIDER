...
export default function SignUpPage({ defaultLogin = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!isSupabaseConfigured) {
        // Fallback for offline / unconfigured demo mode
        localStorage.setItem('spider_demo_user', JSON.stringify({ email: email || 'demo@spider.app', full_name: name || 'Creator' }));
        navigate('/dashboard');
        return;
      }
  ...
  return (
    <div className="min-h-screen w-full flex bg-white font-sans">
      ...
      {/* Right Pane - Form */}
      <div className="w-full lg:w-1/2 bg-[#FAF9F7] min-h-screen lg:h-screen overflow-y-auto custom-scrollbar">
        <div className="min-h-full flex flex-col items-center justify-center p-4 sm:p-6 lg:p-12">
          <div className="w-full max-w-md py-8">
          <Link to="/" className="mb-6 sm:mb-10 inline-block lg:hidden">
          ...
          <button 
            ...
            onClick={async () => {
              if (!isSupabaseConfigured) {
                setErrorMsg('Supabase is not configured yet. Set VITE_SUPABASE_URL and ANON_KEY.');
                return;
              }
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-200 rounded-xl bg-white hover:bg-gray-50 transition-colors shadow-sm mb-6 text-sm font-medium text-gray-700"
          >
          ...
          <div className="relative flex items-center py-5">
            ...
            <span className="flex-shrink-0 mx-2 sm:mx-4 text-gray-400 text-xs sm:text-sm font-medium uppercase tracking-wider">Or {isLogin ? 'log in' : 'register'} with email</span>
            <div className="flex-grow border-t border-gray-200"></div>
          </div>
            ...
            {!isLogin && (
              <div>
                ...
                <input 
                  ...
                  className="w-full text-base px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#9D50BB]/50 focus:border-[#9D50BB] transition-all bg-white" 
                />
              </div>
            )}
            <div>
          ...
          <p className="mt-6 sm:mt-10 text-center text-sm text-gray-600">
